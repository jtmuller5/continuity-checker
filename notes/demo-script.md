...
because the video has no voice track. Every word below is drawn on the screen and repeated in
`out/demo.srt`.
...
| 4 | 0:57 | 10s | card | "What the agent is handed, and what is not". One bible writes the prompt and the questions; the checker never gets the answer key, the canon or the shot's declared state. | **TECH** |
